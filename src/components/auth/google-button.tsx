...
export function GoogleButton() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // After login, Google sends them back to /templates (or /onboarding if new).
  ...
  return (
    <button 
      ...
      disabled={isLoading}
      className="w-full flex items-center justify-center gap-3 bg-white border border-gray-200 text-gray-700 font-medium py-3 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all shadow-xs group disabled:opacity-70"
    >
      {isLoading ? (
        <Loader2 size={20} className="animate-spin text-blue-600" />
      ) : (
        <>
          {/* We use an SVG here to avoid Image component issues if domain config is missing */}
